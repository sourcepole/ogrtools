#!/usr/bin/env python
# Based on ogrinfo.py implementation by Even Rouault included in GDAL/OGR
...
def ogrinfo(bReadOnly = False,
    ...
    global bVerbose_
    global bSummaryOnly_
    global nFetchFID_

    bVerbose_ = bVerbose
    bSummaryOnly_ = bSummaryOnly
    nFetchFID_ = nFetchFID
...
#/* -------------------------------------------------------------------- */
    ...
    if nFetchFID_ == ogr.NullFID and not bSummaryOnly_:
    ...
    elif nFetchFID_ != ogr.NullFID:

        poFeature = poLayer.GetFeature( nFetchFID_ )
        if poFeature is None:
            print( "Unable to locate feature id %d on this layer." % nFetchFID_ )
