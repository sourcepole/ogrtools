...
class ImdParser():
    def __init__(self, fn):
        ...
        self._ns = {'xmlns': re.match(r'^{(.+)}', self._tree.getroot().tag).group(1)}

    def extract_enums(self):
        """Extract Interlis Enumerations"""
        enum_tables = {}
        self._enum_types = self._tree.findall("xmlns:DATASECTION/xmlns:IlisMeta07.ModelData/xmlns:IlisMeta07.ModelData.EnumType", self._ns)
        self._enum_nodes = self._tree.findall("xmlns:DATASECTION/xmlns:IlisMeta07.ModelData/xmlns:IlisMeta07.ModelData.EnumNode", self._ns)
    ...
    def imd_to_dot(self):
        ...
        print "digraph {"
        models = self._tree.findall("xmlns:DATASECTION/xmlns:IlisMeta07.ModelData", self._ns) or []
        modelno = 0
        for model in models:
            taggroup = {}
