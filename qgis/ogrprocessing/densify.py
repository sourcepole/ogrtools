...
class Translator(object):
    ...
    def construct_parser(self):
        ...
        parser.set_defaults(
            verbose=True, driver="ESRI Shapefile", overwrite=True)
    ...
    def open(self):
        ...
        if self.options.layer:
            ...
            if not self.input:
                raise Exception(
                    "The layer '%s' was not found" % self.options.layer)
        else:
        ...
        if not self.out_drv:
            raise Exception(
                "The '%s' driver was not found, did you misspell it or is it not available in this GDAL build?", self.options.driver)
        if not self.out_drv.TestCapability('CreateDataSource'):
            raise Exception(
                "The '%s' driver does not support creating layers, you will have to choose another output driver", self.options.driver)
        if not self.options.output:
            raise Exception("No output layer was specified")
        if self.options.driver == 'ESRI Shapefile':
            path, filename = os.path.split(
                os.path.abspath(self.options.output))
            ...
            if self.options.overwrite:
                ...
                if os.path.exists(os.path.join(path, name,) + ".shp"):
                    raise Exception("The file '%s' already exists, but the overwrite option is not specified" % (
        ...
        self.output = self.out_ds.CreateLayer(self.options.output,
                                              geom_type=self.input.GetLayerDefn(
                                              ).GetGeomType(),
                                              srs=self.out_srs)
